...
(function($, window, document, undefined) {
    ...
    $.fn.imageuploadify = function(opts) {
        ...
        this.each(function() {
            ...
            let totalFiles = [];
            let counter = 0;
            let dragbox = $('<div class="imageuploadify well"><div class="imageuploadify-overlay"><i class="fa fa-picture-o"></i></div><div class="imageuploadify-images-list text-center"><button type="button"class="btn btn-default">' + title + '</button></div></div>');
            ...
            var oldImages = $(self).attr('images');
            if(oldImages) {
                var imgs = oldImages.split(";");
                if(imgs.length) {
                    imgs.forEach(function (img) {
                        const width = dragbox.width();
                        const boxesNb = Math.floor(width / 100);
                        const marginSize = Math.floor((width - (boxesNb * 100)) / (boxesNb + 1));
                        let icontainer = $(`<div class='imageuploadify-container'><button type='button'class='btn btn-danger glyphicon glyphicon-remove'></button><div class='imageuploadify-details'><span></span><span></span><span></span></div></div>`);
                        let idetails = icontainer.find(".imageuploadify-details");
                        let ideleteBtn = icontainer.find("button");
                        ideleteBtn.on("click", function() {
                            if(confirm('Are you sure? This will delete the image from server.')) {
                                if($(self).parent().children('.TestClass').val() == 2){
                                    if(window.countImage > 0) {
                                        window.countImage = parseInt(window.countImage) - 1;
                                    }
                                }
                                if($(self).parent().children('.TestClass').val() == 1){
                                    if(window.featureImageCount > 0){
                                        window.featureImageCount = parseInt(window.featureImageCount) - 1;
                                    }
                                }
                                $.post($(self).attr('del-src'), {_token: $("meta[name=\"csrf-token\"]")[0].content, img: img});
                                $(this.parentElement).remove();
                            }
                        });
                        icontainer.css("margin-left", marginSize + "px");
                        idetails.hover(function() {
                            $(this).css("opacity", "1");
                        }).mouseleave(function() {
                            $(this).css("opacity", "0");
                        });
                ...
                for (let index = 0; index < files.length; ++index) {
                    if (!accept || compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
                        ...
                        if ($(self).attr("multiple")) {
                            ...
                            $(self).parent().find(".imageuploadify-container").remove()
                            totalFiles = [{
                                id: id,
                                file: files[index]
                            }];
